% Two guards, and both are needed.  The first chunk of this longtable -- caption, head,
% and the break-protected first panel -- is one unbreakable ~308pt block, so on a page
% with less room than that it runs into the bottom margin, silently: \Needspace* asks
% for the room.  But \Needspace* alone is not enough here, because the table is
% \input after a queue of figure floats: it breaks the page, the pending figure is then
% placed at the top of the fresh page and eats the reserved space, and longtable, whose
% only breakpoint is now its very start, emits \endfoot and then \endhead -- printing
% \"continued from the previous page\" ABOVE the table's own caption.  \FloatBarrier
% drains the queue first so that cannot happen.  Measured: without the barrier the
% manuscript prints the continuation head above the caption on the table's first page;
% with it, both documents are clean at no extra page.
\FloatBarrier
...
\caption{\textbf{Ablations} on the homogeneous autoregressive design ($d=20$, $\kappa=2.92$, $R=250$). The three coverage columns --- \emph{2-scale}, \emph{BM} and \emph{plug-in} --- are coverage of nominal 95\% intervals using the two-scale, plain batch-means and i.i.d.\ plug-in variance respectively; \emph{eff} is the empirical variance of the point estimate divided by its asymptotic value ($1.00$ is efficient), in scientific notation where the unsafeguarded step diverges; and $r_j$ is the free block-adequacy diagnostic, an estimate of the relative block-length bias of plain batch means. Panel (c$'$) is on the regime-switching design and is why the default warm-up is $c_{\mathrm w}=200$ rather than $50$ (\Cref{app:abl}). Panel (f) varies the Bernoulli retention probability $p$ at gap $m=1$ and then the gap $m$ at $p=1$, so the shared baseline $m=1,\ p=1$ appears in both sub-sweeps. In panel~(i) the designs are abbreviated AR (autoregressive), AR-s (strongly autoregressive) and Mkv (regime-switching), and the variants are: \emph{none}, the raw $1/t$ step; \emph{cap}, the step-length safeguard of \eqref{eq:step} at $c_D=1$ (our default), with \emph{cap 0.25} and \emph{cap 4} changing $c_D$ by a factor of four either way; \emph{shift}, the rejected schedule shift of \Cref{app:t0}; and \emph{both}, the two together. The number of blocks out of $10{,}000$ on which the safeguard acted is reported in \Cref{app:abl} and in \Cref{tab:hard}. Panel (j) is on a short stream ($N=4000$, $d=11$, the length of a real-data evaluation segment) and varies the cap on the warm-up as a fraction of the stream; $\varpi_{\mathrm w}=1$ means uncapped, which spends $2200$ of the $4000$ observations on curvature and leaves $45$ blocked steps.}
\label{tab:ablation}\\
\toprule
panel & setting & eff & 2-scale & BM & plug-in & $r_j$ \\
\midrule
\endfirsthead
\multicolumn{7}{l}{\emph{\tablename~\thetable, continued from the previous page.}}\\
\toprule
panel & setting & eff & 2-scale & BM & plug-in & $r_j$ \\
\midrule
\endhead
% no \midrule here: the inter-panel rule that lands at a break is already there, and a
% second one 2.5pt below it prints as a doubled rule
