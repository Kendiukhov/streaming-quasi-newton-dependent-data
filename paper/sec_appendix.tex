...
\[
\gamma_t^2\opnorm{P_{t-1}}^2=O(t^{2q_{\mathrm r}-2}),
\qquad
\E\norm{\gb_t(\theta_{t-1})}^2\le C+C'(F(\theta_{t-1})-F(\ts)),
\]
the first by \Cref{lem:ridge} with $q_{\mathrm r}<1/2$ and the second by \Cref{as:mom} and
Jensen. The cross term is the one that
...
\[
\norm{\theta_T-\ts}=O\big(T^{-1/2}\log^{1+\varpi}T\big)
\quad\text{almost surely, for every }\varpi>0.
\]
\end{lemma}
