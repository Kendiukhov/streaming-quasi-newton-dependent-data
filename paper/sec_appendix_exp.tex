...
This design is also the one that forces the longer warm-up. With
$\Prob(s_t=s_{t-1})=0.97$ the chain dwells in a regime for about $33$ observations, so a block
of $b=20$ consecutive observations almost always lies inside a single regime and its empirical
curvature is nearly rank one. A warm-up of $50d$ \emph{consecutive} observations then spans
only about thirty regime visits, leaves $\Hb$ rank-starved, and the $1/t$ schedule diverges
outright (\Cref{tab:ablation}(c$'$)); $200d$ suffices.
...
$2\Phi\big(z_{\alpha/2}/\sqrt{1-1/R}\big)-1$, which is what the header of \Cref{tab:real}
reports. Assumption (iii) requires the full-stream estimator to be (approximately) the equally
weighted average of the segment estimators, which holds when the segments have equal length
and similar $\widehat H$; (i) is the same approximation that makes segments usable as
replicates at all. The correction is small---it raises the target from $0.950$ to about
$0.955$ at $R=8$---and we report the raw coverage alongside it.
...
parameter, retained window dyadic (last half to last three quarters of blocks).
...
\paragraph{The iterate-path comparator.} \Cref{fig:lrvrate}(a) compares against overlapping
