...
$95\%$ intervals therefore converge to a coverage below $95\%$ and stay there. On the air-quality stream we study
the median estimated inflation is $\RealKappaAirq$, which by that formula pins the limiting
coverage of a nominal $95\%$ interval at $\RealPredAirq$.
...
$S$ is estimated at $\widetilde O(N^{-1/2})$, against the $N^{-1/8}$ proved for the best published
iterate-based estimators under dependence; for reference, estimators built from an iterate path
without curvature information cannot beat $N^{-1/4}$ even under independence, at any admissible
step-size exponent. The reported sandwich converges at $N^{-q}$ for any $q<1/2$. We also give the exact asymptotic
coverage of dependence-blind intervals, $2\Phi(z_{\alpha/2}/\sqrt\kappa)-1$ in one
variance-inflation factor $\kappa$, and verify it with no fitted constants.

Blocking is not free of consequences, and we report the one it has. Each step multiplies the
block's average gradient by a running estimate of the inverse curvature, and the step contracts
the error only while the step size times the size of that product stays below $2$. On
independent data the product is close to the identity and the condition is automatic; on a
dependent stream a block of $b$ observations can span far fewer than $b$ distinct covariate
directions, the block's own curvature is then nearly singular, and a full-length first step
overshoots by orders of magnitude --- without a safeguard our own method's variance on a
regime-switching design is $\ShiftEffMarkovNone$ times the efficient value and individual runs
diverge. We state the condition, give an $O(d)$ step-length safeguard that fixes it, and show the
safeguard is inactive after finitely many steps, so it changes no asymptotic statement. On autoregressive, Markov-modulated and dependent-logistic designs with $\kappa$
between $\KappaAllLo$ and $\KappaAllHi$, and on hourly traffic and air-quality streams, our
nominal $95\%$ intervals cover $\CovOursLo$--$\CovOursHi$ where dependence-blind intervals
cover $\CovBlindLo$--$\CovBlindHi$, at $\WidthRatioLo$--$\WidthRatioHi$ times the width. The low
end of that range is two designs on which no streaming point estimate has reached its asymptotic
regime at this sample size, and there the right comparison is not the nominal level but an
infeasible interval built from the exact $H^{-1}SH^{-1}$ at the same iterate. Against that
benchmark our coverage is within $\HardMaxGapMarkov$ of the oracle's at every sample size in a
sweep on the regime-switching design. On the dependent-logistic design there is a real gap, and
it is ours: $\HardLogitGapSmall$ at $N=\HardNSmall$, falling to $\HardLogitGapBig$ at
$N=\HardNBig$. We trace it to a finite-sample bias in the fitted standard error, caused by
averaging curvature along the optimisation path, and we show it closing rather than assert it
away. On both designs our interval and the infeasible one rise towards nominal together; the
dependence-blind interval does not.
\end{abstract}
...
\begin{figure}[t]
\centering
\IfFileExists{../figures/fig8_hard_design.pdf}{\includegraphics[width=\linewidth]{../figures/fig8_hard_design.pdf}}{\fbox{\parbox{0.9\linewidth}{\centering\vspace{2em}\texttt{\detokenize{fig8_hard_design}} pending\vspace{2em}}}}
